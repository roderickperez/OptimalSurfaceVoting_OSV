...
## Notebook Parity (2026 Update)

- Training and prediction notebooks are aligned in section ordering and instructional text with the Keras Updated and TensorFlow notebook flows.
- The workflow mirrors the original Keras reference process while keeping framework-specific code in PyTorch.
- Training epochs are aligned to `25` for parity with the original Keras setup.

## GPU Environment Check

Before running notebooks, verify CUDA availability in the active environment:

```bash
python3 -c "import torch; print(torch.__version__); print('CUDA:', torch.cuda.is_available(), 'devices:', torch.cuda.device_count())"
```
